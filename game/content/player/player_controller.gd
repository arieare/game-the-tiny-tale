...
@onready var move_walk_node = $movement/walk
@onready var move_levitate_node = $movement/levitation
@onready var move_upright_node = $movement/upright

func _ready() -> void:
	move_walk_node.assert_parent(self)
	move_levitate_node.assert_parent(self)
	move_upright_node.assert_parent(self)

func _physics_process(delta: float) -> void:
	move_levitate_node.levitate(delta)
	move_walk_node.walk(input_node.get_input(), delta)
	
	if input_node.get_input() == Vector3.ZERO: ## Idle
		move_upright_node.maintain_upright(delta) 
	
	if Input.is_action_just_pressed("ui_accept") and move_levitate_node.is_on_floor:
