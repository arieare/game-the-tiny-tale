...
func trauma_shake(delta):
	if trauma:
		...
		parent_cam.rotation.z = max_roll * amount * randf_range(-1, 1)
		parent_cam.h_offset = max_offset.x * amount * randf_range(-1, 1)
		parent_cam.v_offset = max_offset.y * amount * randf_range(-1, 1)
